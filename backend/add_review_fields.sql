-- Migration: Add Review Flags to Transaction Ledger
-- Run this script in your Supabase SQL Editor (https://supabase.com/dashboard/project/_/sql)

-- 1. Add review status columns to finance_transactions
ALTER TABLE public.finance_transactions 
ADD COLUMN IF NOT EXISTS is_auto_filled BOOLEAN DEFAULT FALSE,
ADD COLUMN IF NOT EXISTS needs_review BOOLEAN DEFAULT FALSE;

-- 2. Index to speed up queries on transactions needing review
CREATE INDEX IF NOT EXISTS idx_finance_transactions_needs_review 
ON public.finance_transactions(user_id, needs_review) 
WHERE needs_review = TRUE;

-- 3. Update the Reconciliation Stored Procedure for Transactions
...
DECLARE
    ...
    tx_is_auto_filled BOOLEAN;
    tx_needs_review BOOLEAN;
...
BEGIN
    ...
    FOR rec IN (
        ...
        BEGIN
            ...
            tx_is_auto_filled := COALESCE((rec.raw_data->>'is_auto_filled')::BOOLEAN, FALSE);
            tx_needs_review := COALESCE((rec.raw_data->>'needs_review')::BOOLEAN, FALSE);
            ...
            ELSE
                INSERT INTO public.finance_transactions (
                    user_id, date, amount, type, category, method, description, source, external_ref_id, is_auto_filled, needs_review
                ) VALUES (
                    tx_user_id, tx_date, tx_amount, tx_type, tx_category, tx_method, tx_desc, tx_source, comp_hash, tx_is_auto_filled, tx_needs_review
                );

...
router.get('/pnl-pdf', requireAuth, async (req, res) => {
  try {
    ...
    doc.fillColor(pnlColor).fontSize(14).font('Helvetica-Bold').text(`${pnlSign}₹${summary.total_realized_pnl.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, 70, y + 30);

    doc.fillColor('#64748b').fontSize(8).font('Helvetica').text('SHORT-TERM GAINS (STCG)', 200, y + 15);
    doc.fillColor(summary.stcg >= 0 ? '#0f172a' : '#ef4444').fontSize(11).font('Helvetica-Bold').text(`₹${summary.stcg.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, 200, y + 30);

    doc.fillColor('#64748b').fontSize(8).font('Helvetica').text('LONG-TERM GAINS (LTCG)', 330, y + 15);
    doc.fillColor(summary.ltcg >= 0 ? '#0f172a' : '#ef4444').fontSize(11).font('Helvetica-Bold').text(`₹${summary.ltcg.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, 330, y + 30);

    doc.fillColor('#64748b').fontSize(8).font('Helvetica').text('EST. INCOME TAX LIABILITY', 460, y + 15);
    doc.fillColor(totalTax > 0 ? '#f59e0b' : '#64748b').fontSize(11).font('Helvetica-Bold').text(`₹${totalTax.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, 460, y + 30);
    ...
    if (monthEntries.length === 0) {
      ...
      monthEntries.forEach(([month, mData]) => {
        ...
        const mCols = [
          ...
          { text: `${sign}₹${mData.pnl.toLocaleString('en-IN', { minimumFractionDigits: 2 })}`, width: 160, align: 'right' }
        ];
    ...
    const stockCols = [
      ...
      { text: 'BUY VALUE (₹)', width: 100, align: 'right' },
      { text: 'SELL VALUE (₹)', width: 100, align: 'right' },
      { text: 'STCG (₹)', width: 65, align: 'right' },
      { text: 'LTCG (₹)', width: 65, align: 'right' },
      { text: 'NET P&L (₹)', width: 94, align: 'right' }
    ];

...
/**
 * POST /api/export/markdown-pdf
 * Converts arbitrary markdown generated by the assistant into a formatted PDF document.
 */
router.post('/markdown-pdf', requireAuth, async (req, res) => {
  try {
...
/**
 * POST /api/export/markdown-csv
 * Extracts the first table from assistant markdown and downloads it as a CSV file.
 */
router.post('/markdown-csv', requireAuth, async (req, res) => {
  try {
    const { markdown } = req.body;
    if (!markdown) {
      return res.status(400).json({ error: 'Markdown content is required.' });
